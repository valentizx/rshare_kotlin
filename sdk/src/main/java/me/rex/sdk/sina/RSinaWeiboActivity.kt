...
class RSinaWeiboActivity : Activity(), WbShareCallback {
    ...
    private fun handleIntent() {
        ...
        when (type) {
            ShareContentType.Text -> {
                mHandler.shareMessage(helper.textMessage, true)
            }
            ShareContentType.Photo -> {
                mHandler.shareMessage(helper.imageMessage, true)
            }
            ShareContentType.Webpage -> {
                mHandler.shareMessage(helper.webMessage , true)
            }
            ShareContentType.Video -> {
                mHandler.shareMessage(helper.videoMessage, true)
            }
        }
    }
